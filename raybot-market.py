...
# ---------------------------
# Market fetch helper (generic)
# ---------------------------
def fetch_history(ticker='GC=F', days=365, interval="1d") -> pd.DataFrame:
    if YahooTicker is None:
        raise RuntimeError("yahooquery not installed")
    end = datetime.utcnow()
    start = end - timedelta(days=days)
    logger.info(f"Fetching data for {ticker} from {start} to {end}")
    tq = YahooTicker(ticker)
    ...
    for col in required:
        if col not in raw.columns:
            raw[col] = 0.0
    logger.info(f"Fetched {len(raw)} records for {ticker}")
    return raw[required]
...
# --- New: ticker dropdown (add-on) ---
TICKERS = [
    'EURUSD=X', 'GBPUSD=X', 'USDJPY=X', 'USDCHF=X', 'USDCAD=X', 'AUDUSD=X', 'NZDUSD=X',
    'CL=F', 'USO', 'BTC-USD', 'GC=F'
]
selected_ticker = st.sidebar.selectbox("Select ticker to fetch", TICKERS, index=TICKERS.index('GC=F'))
# -------------------------------------
...
# Fetch market data (generic)
if st.button(f"Fetch latest data ({selected_ticker})"):
    try:
        df = fetch_history(ticker=selected_ticker, days=365, interval="1d")
        if df.empty:
            st.error("No data returned")
        else:
            st.session_state.market_df = df
            st.success(f"Fetched {len(df)} bars for {selected_ticker}")
